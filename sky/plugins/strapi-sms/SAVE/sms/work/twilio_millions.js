...
const running_result = async function(rec, task_id) {
    let error = [ ]
    let success = [ ]
    ...
    await db.task_result(task_id, success, error)
} 

// 发送
const running_task = async function(task, _factory, _sender) {
    ...
    if (res) { await running_result(res, task.id) }
